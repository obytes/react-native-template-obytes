...
**New files:**
- `src/translations/en.json` — add `settings.finances`, `settings.accounts`
- `src/translations/ar.json` — add same keys
...
```json
{
  "onboarding": { "message": "مرحبا بكم في موقع تطبيق obytes" },
  "settings": {
    "about": "حول التطبيق ",
    "accounts": "الحسابات",
...
git commit -m "feat(i18n): add finances and accounts translation keys"
```
