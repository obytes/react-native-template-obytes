...
**New files:**
- `src/translations/en.json` — migrate to Spanish, add `settings.finances`, `settings.accounts`
- `src/translations/ar.json` — mirror same Spanish strings
...
> **Note:** The project uses Spanish as its only language. Both `en.json` and `ar.json` are kept in sync with Spanish strings so the i18n system works regardless of device locale.

**Files:**
- Modify: `src/translations/en.json`
- Modify: `src/translations/ar.json`

- [ ] **Step 1: Add keys to en.json (Spanish)**

```json
{
  "onboarding": { "message": "Welcome to obytes app site" },
  "settings": {
    "about": "Acerca de",
    "accounts": "Cuentas",
...
- [ ] **Step 2: Mirror same keys to ar.json (also Spanish)**
...
- [ ] **Step 3: Verify TypeScript accepts new keys**
...
git commit -m "feat(i18n): migrate all translations to Spanish, add finances and accounts keys"
```
