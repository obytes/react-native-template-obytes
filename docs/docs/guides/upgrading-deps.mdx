...
In order to take advantage of the latest improvements in the starter project, you will need to compare your `osMetadata.initVersion` with the latest release on GitHub, and manually apply any necessary changes to your project.
:::
...
yarn doctor ## check for any issues with the dependencies you added to your project
yarn lint ## linting
...
yarn start --cc ## start the server
```
...
npx expo-doctor
or
yarn expo doctor ## a script inside the starter
```
...
Now it time to make sure everything is still working as expected 😀:
...
:::tip
Upgrading your project on a regular basis can make the process much easier. We recommend upgrading your project  a month after each new Expo  major version is released. This will help you stay up to date with the latest features and security updates while minimizing the risk of encountering compatibility issues with older versions of Expo.
:::
...
While Expo upgrade will help you update most native dependencies automatically through the process outlined above, it's important to keep in mind that non-native dependencies(JS only) may also need to be updated too.


This time you will use the power of yarn to update dependencies to last version and then run our checks to make sure everything is working as expected.
...
2. run `yarn doctor` to revert any changes that is not in sync with the Expo version you are using.
...
If you are lucky enough and everything work as expected without any issues, you can now update the rest of the dependencies.


:::tip
In case you application is not tested enough,it's a good idea to take some time to go throw the application and ensure that the most critical features are working correctly before committing any changes.
:::
