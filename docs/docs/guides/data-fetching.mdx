...
To make writing queries and mutation even easier we are using [react-query-kit](https://github.com/liaoliao666/react-query-kit) a simple toolkit for react-query that make ReactQuery reusable and typesafe with less boilerplate.
...
2. Create a type for your  `Response` and `Variables` if needed to make sure that you are getting the right data from the server.(eg. `Post` type)
3. Use `createQuery` function from `react-query-kit` to create a query hook that will fetch the data from the server. (eg. `usePosts` hook)
...
As you may notice in the code above using `createQuery` function is very simple and easy and accept 3 params: `queryKey`, `queryFn` and `options`. read more about `createQuery` [here]

:::tip
Use the snippet to generate query in no time using vscode : [🔗](#create-api-hooks-with-vs-code-snippets)
...
We are going to use the same steps as we did in the previous section to create the `usePost` hook. The only difference is that we will use the `id` as a variables to fetch the post.
...
To implement the create post feature we will need to use the `createMutation` function from `react-query-kit` following the steps below:

1. Create a new file called `add-post.ts` inside `src/api/posts`
2. Create a type mutation variables and response to make sure that you are sending the right data to the server.( `Variables` type)
3. use `createMutation` function from `react-query-kit` to create a mutation hook that will send the data to the server. ( `useAddPost` hook).
