...
Since we’re using [nativewind](https://www.nativewind.dev/) (that use Tailwind CSS under the hood) and react-navigation we let them handle applying it ,and we just take care of the colors we want .  
...
we use `loadSelectedTheme` function to load the theme from the storage if there's a theme saved in the storage, otherwise we let nativwind use the default theme (system).
and for setting selected theme we use `useSelectedTheme` hook to set the theme in the storage and update the color scheme of the app.
...
to add the values for the light mode you can simply write them direlty in your component class and for the dark mode you can use the **dark:** variant.
...
```
if you want to use style prop you can use the `useColorScheme` hook to get the current color scheme and use it to apply the style you want but **for the most of the cases you don't need it** dark: variant will do the job for you.
