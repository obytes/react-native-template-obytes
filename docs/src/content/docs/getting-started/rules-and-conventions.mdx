...
As the name suggest, this hook will run after each merge and it will check if there is any changed in `bun.lockb` and if there is any, it will run `bun install` to make sure the dependencies are up to date.
