...
If you run `bun run app-release`, it will ask you to choose the type of release (major, minor, patch). based on the type of release, it will update the version inside `package.json`.
...
So technically, we just need to run `bun run app-release` and it will handle the rest for us. from updating native app version to pushing a new tag to GitHub.
